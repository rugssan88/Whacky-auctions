...
function header() {
  return `<header class="topbar"><div class="container nav"><a class="brand" href="/" data-link aria-label="Whacky Auctions home"><img class="brand-logo" src="/branding/whacky-auctions-logo.png" alt="Whacky Auctions" /></a>
...
function footer() {
  return `<footer class="footer"><div class="container footer-grid"><div><div class="brand footer-brand"><img class="brand-logo" src="/branding/whacky-auctions-logo.png" alt="Whacky Auctions" /></div><p class="muted small">A proudly South African auction room. Lekker goods, honest notes and no funny business.</p><p class="small">Whacky Auctions PTY LTD · Florida, Gauteng</p><div class="footer-socials" aria-label="Whacky Auctions social media"><a href="https://www.instagram.com/whacky.auctions/" target="_blank" rel="noopener noreferrer"><span>Instagram</span><b>@whacky.auctions</b></a><a href="https://www.tiktok.com/@whacky.auctions.za" target="_blank" rel="noopener noreferrer"><span>TikTok</span><b>@whacky.auctions.za</b></a><a href="https://www.facebook.com/profile.php?id=61594103503157" target="_blank" rel="noopener noreferrer"><span>Facebook</span><b>Whacky Auctions</b></a></div></div><div><b>Explore</b><a href="/" data-link>Home</a><a href="/auctions" data-link>Auctions</a><a href="/how-it-works" data-link>How it works</a><a href="/about" data-link>About us</a></div><div><b>Help & account</b><a href="/contact" data-link>Contact us</a><a href="/join" data-link>Early access</a><a href="/account" data-link>My account</a><a href="/install" data-link>Install the app</a></div><div><b>Trust & legal</b><a href="/legal" data-link>Legal centre</a><a href="/legal/privacy-policy.pdf" target="_blank">Privacy Policy</a><a href="/legal/terms-auction-rules.pdf" target="_blank">Terms & Auction Rules</a><a href="https://pay.yoco.com/whacky-auctions" target="_blank" rel="noopener">Secure payments by Yoco ↗</a></div></div><div class="container footer-bottom"><span>© ${new Date().getFullYear()} Whacky Auctions</span><a href="mailto:info@whackyauctions.co.za">info@whackyauctions.co.za</a></div></footer>`;
}
...
function bidGate(a) {
  ...
  if (!state.me.verified)
    return '<div class="notice" style="margin-top:12px">Complete your once-off bidder verification in your account before bidding.</div>';
...
async function verifyBidderPage() {
  ...
  const activationNotice = verification.freeActivationEligible
    ? verification.freeActivationReason === "cloud9"
      ? '<div class="notice good"><b>CLOUD9 launch perk: FREE</b><br>You are within the first 20 registrations using CLOUD9. Your bidder-activation fee is waived.</div><button class="btn btn-primary">Activate my bidder status free</button>'
      : '<div class="notice good"><b>Founding-member activation: FREE</b><br>You are among the first 100 Early Access signups. Your normal R10 activation fee is waived.</div><button class="btn btn-primary">Activate my bidder status free</button>'
    : verification.promoCode === "CLOUD9" && Number(verification.amountCents) === 500
      ? '<div class="notice good"><b>CLOUD9 promo: R5 once off</b><br>Your CLOUD9 code reduces the standard R10 bidder-activation fee to R5. Yoco securely processes the payment.</div><button class="btn btn-primary">Continue to secure R5 payment</button>'
      : '<div class="notice"><b>Once-off activation: R10</b><br>Yoco securely processes the payment. Successful payment activates your verified bidder status automatically. Your card is not saved and automatic debits are not authorised.</div><button class="btn btn-primary">Continue to secure R10 payment</button>';
  return `${header()}<main class="section"><div class="container"><div class="section-head"><div><div class="label">Separate bidder registration</div><h2>Become a verified bidder</h2></div><a class="btn btn-secondary" href="/profile" data-link>Back to account</a></div><div class="panel" style="max-width:720px"><p>This is optional. Complete it only if you want to bid.</p><div class="kv"><div><div class="label">Name & surname</div><b>${esc(state.me.firstName)} ${esc(state.me.lastName)}</b></div><div><div class="label">Contact number</div><b>${esc(state.me.mobile || "")}</b></div></div><form id="verifyBidderForm" class="admin-form" style="margin-top:18px"><div class="field"><label>ID number</label><input class="input" name="idNumber" autocomplete="off" required></div>${activationNotice}</form></div></div></main>${footer()}`;
}
function legal() {
...
async function adminRecords() {
  const [d, referralData] = await Promise.all([
    api("admin/founder-email-preview"),
    api("admin/referrals").catch(() => ({ referrals: [] })),
  ]);
