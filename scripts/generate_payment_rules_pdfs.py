...
master=[]
master+=heading('APP TERMS & CONDITIONS<br/>AND MASTER RULES OF AUCTION','Version 1.2 | Effective 18 September 2026')
master+=[box('<b>IMPORTANT PAYMENT TERMS</b><br/>A once-off R10 Yoco payment is normally required to activate bidder verification. As a founding-member promotion, the first 100 recorded Early Access signups receive this activation free. A winning bidder must pay the full displayed amount through the separate winning-order checkout within 2 hours, unless the auction-specific schedule lawfully states another period. A verification payment does not authorise Whacky Auctions to save or automatically debit a card.') ,Spacer(1,8)]
sections=[
...
('Eligibility and bidder registration','Bidders must be 18 or older and legally capable of contracting. Bidder verification collects name, surname, contact number and ID number, together with age confirmation, login, IP and payment status where needed for security and lawful records. A bid from an unregistered or unverified person is invalid. Authority is required when bidding for another person or entity.'),
('Bidder-verification activation','After completing registration, a bidder normally makes a once-off R10 payment through the Yoco-hosted checkout before bidding privileges activate. Whacky Auctions waives this activation fee for the first 100 recorded Early Access signups. Promotional eligibility is determined by the platform signup record and does not remove identity or eligibility checks. Where paid, the fee is non-refundable once verification succeeds except where law requires otherwise. Card data is handled by Yoco; Whacky Auctions does not receive or store the full card number or CVV. This payment is not a deposit and does not authorise recurring or automatic charges.'),
...
doc=SimpleDocTemplate(OUT+'terms-auction-rules.pdf',pagesize=letter,rightMargin=18*mm,leftMargin=18*mm,topMargin=18*mm,bottomMargin=20*mm,title='Whacky Auctions App Terms and Master Rules of Auction v1.2',author='Whacky Auctions PTY LTD')
doc.build(master,onFirstPage=footer,onLaterPages=footer)

live=[]; live+=heading('STANDARD LIVE AUCTION RULES - 2026','Version 1.3 | Effective 18 September 2026')
...
live+=[KeepTogether(section(1,'Bidder verification','Only registered users aged 18 or older whose bidder activation has been confirmed may bid. Activation normally costs R10 once off through Yoco; the fee is waived for the first 100 recorded Early Access signups. Any verification payment is not a deposit and does not authorise saved-card or automatic charges.'))]
...
live+=[KeepTogether(section(6,'Incorporated master rules','These standard live rules must be read with the Whacky Auctions App Terms & Master Rules of Auction v1.2. Auction-specific listing terms prevail only to the extent they are lawful and were properly disclosed before bidding.'))]
...
doc2=SimpleDocTemplate(OUT+'standard-live-auction-rules-2026.pdf',pagesize=letter,rightMargin=18*mm,leftMargin=18*mm,topMargin=18*mm,bottomMargin=20*mm,title='Whacky Auctions Standard Live Auction Rules 2026 v1.3',author='Whacky Auctions PTY LTD')
