...
live=[]; live+=heading('LIVE AUCTION RULES & SCHEDULE TEMPLATE','Version 1.1 | Updated 14 September 2026')
live+=[box('<b>COMPLETE AND PUBLISH THIS SCHEDULE BEFORE BIDDING</b><br/>Replace every bracketed field. Publish for the period required by law and keep the published version with the auction record.'),Spacer(1,10)]
rows=[('Auction title / ID','[INSERT]'),('Online place / URL','[INSERT]'),('Auctioneer and registration details','[INSERT]'),('Rules publication date and time','[INSERT]'),('Auction start','[INSERT]'),('Scheduled close','[INSERT]'),('Reserve status','[WITH RESERVE / WITHOUT RESERVE]'),('Opening bid and increment','[INSERT]'),('Buyer premium and VAT','[INSERT]'),('Payment deadline','2 hours after completion, unless lawfully replaced here: [INSERT]'),('Inspection','[INSERT]'),('Collection / delivery','[INSERT]'),('Storage charges','[INSERT]')]
...
live+=section(1,'Bidder verification','Only registered users aged 18 or older whose once-off R10 Yoco bidder-verification payment has been confirmed may bid. The verification payment is not a deposit and does not authorise saved-card or automatic charges.')
live+=section(2,'Bidding and soft close','Each bid is binding when the sale is completed. A valid bid in the final 2 minutes restores a full 2-minute window; repeated late bids repeat the extension. Lawful pre-completion bid retractions will be processed and recorded.')
live+=section(3,'Payment','The winning bidder must pay the full displayed amount through the winning-order Yoco checkout by the deadline above. Payment is complete only when confirmed by the provider. No goods are released before cleared payment.')
live+=section(4,'Default','If cleared payment is not received by the deadline, Whacky Auctions may cancel the sale, restrict the bidder and re-offer the lot. Any default fee is capped at the lesser of 10% of the purchase price or the auction and additional costs recoverable under applicable auction regulations.')
live+=section(5,'Condition and collection','The lot condition, known defects, inspection opportunity and collection/delivery terms are those disclosed in the listing and this schedule. Goods are sold as-is to the extent permitted by law; mandatory rights remain unaffected.')
live+=section(6,'Incorporated master rules','This schedule must be read with the Whacky Auctions App Terms & Master Rules of Auction v1.1. The auction-specific schedule prevails only to the extent it is lawful and was properly disclosed.')
live+=[Spacer(1,10),Paragraph('<b>Signed / approved for publication</b>',styles['H1WA']),Table([['Auctioneer: ______________________________','Date/time: __________________'],['Whacky Auctions representative: ____________','Date/time: __________________']],colWidths=[100*mm,70*mm],rowHeights=[13*mm,13*mm],style=TableStyle([('BOX',(0,0),(-1,-1),.5,colors.HexColor('#D8CCC1')),('INNERGRID',(0,0),(-1,-1),.5,colors.HexColor('#D8CCC1')),('VALIGN',(0,0),(-1,-1),'MIDDLE'),('LEFTPADDING',(0,0),(-1,-1),7)]))]
doc2=SimpleDocTemplate(OUT+'live-auction-rules-template.pdf',pagesize=letter,rightMargin=18*mm,leftMargin=18*mm,topMargin=18*mm,bottomMargin=20*mm,title='Whacky Auctions Live Auction Rules Template v1.1',author='Whacky Auctions PTY LTD')
